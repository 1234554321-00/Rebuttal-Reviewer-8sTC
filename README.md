...
Parameter counts from network architecture (input_dim × hidden_dim × layers), AUROC from Table 1
...
All components show ~4× reduction due to embedding dimension reduction (h'=64 vs h=128).
...
Speedup remains consistent (2.3-2.4×) across diverse graph sizes, confirming scalability of our approach per Theorem 7.
...
Framework excels at combined anomalies (92.1% AUROC), validating our multi-scale perturbation strategy. The dual scoring mechanism (reconstruction + distributional) captures both feature and structural irregularities.
